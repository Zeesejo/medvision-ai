...
This approach is inspired by:
  ...
  - Zhou et al., "Self Pre-Training with Masked Autoencoders for Medical Image
    Analysis", ISBI 2023
  - Xiao et al., "Delving into the Frequency Domain for Medical Image Analysis"

Usage:
  python src/ssl/mae_pretrain.py \
      --img_dir  data/images \
      --save_dir results/ssl \
      --epochs   100 \
      --batch_size 64

The saved checkpoint contains {"backbone": backbone_state_dict, "epoch": N}
and can be loaded by train_v3.py via --ssl_checkpoint.
"""
...
# ───────────────────────────────────────────────────────────────
IMG_SIZE   = 224   # MAE pretraining uses 224 (patch-friendly)
PATCH_SIZE = 16    # 14x14 = 196 patches
MASK_RATIO = 0.75  # standard MAE masking ratio


# ── unlabeled CXR dataset for SSL ───────────────────────────────────────
class UnlabeledCXRDataset(Dataset):
    """
    Loads all images recursively from img_dir (no labels needed).
...
def anatomy_guided_mask(num_patches: int, img_size: int, patch_size: int,
    ...
    Novel contribution: biased masking toward lung fields forces the
    backbone to learn anatomy-specific representations, improving
    downstream rare-disease detection AUC.
    """
    n  = int(math.sqrt(num_patches))   # patches per side
    weights = torch.ones(num_patches)

    # Lung region: rows 20%-80%, cols 15%-85% of the patch grid
    row_lo, row_hi = int(0.20 * n), int(0.80 * n)
    col_lo, col_hi = int(0.15 * n), int(0.85 * n)
    for r in range(n):
        for c in range(n):
            if row_lo <= r < row_hi and col_lo <= c < col_hi:
                weights[r * n + c] = 2.0    # 2x probability in lung region
...
# ── simple patch-based MAE ──────────────────────────────────────────────────
class LightweightMAE(nn.Module):
    """
    Patch-based MAE using ResNet-50 as encoder (CNN-MAE variant).
    Decoder is a lightweight 2-layer MLP that reconstructs pixel patches.

    We use ResNet-50 (not ViT) as encoder because:
    - It aligns with our DenseNet-121 fine-tuning backbone (both CNN)
    - Transfer from CNN-MAE to CNN classifier is more direct
    - Lower compute cost for pretraining without GPU cluster

    The novel frequency-domain loss is applied on top of pixel MSE.
    """
    def __init__(self, img_size: int = 224, patch_size: int = 16):
        ...
        self.patch_dim   = 3 * patch_size * patch_size   # RGB pixels per patch

        # Encoder: ResNet-50 backbone (feature extractor)
        ...
        # Decoder: lightweight MLP per patch
    ...
    def forward(self, imgs: torch.Tensor, mask: torch.Tensor):
        """
        imgs : (B, 3, H, W)
        mask : (N,) bool  — True = masked patch (target for reconstruction)
        Returns (pred_patches, target_patches, mask)
        """
        B = imgs.size(0)
        target_patches = self.patchify(imgs)   # (B, N, patch_dim)

        # Mask input: zero out masked patches in image space before encoding
        p     = self.patch_size
        n     = self.img_size // p
...
def freq_reconstruction_loss(pred: torch.Tensor, target: torch.Tensor,
    ...
    L = MSE(pred, target) on masked patches
      + freq_weight * |FFT(pred) - FFT(target)| magnitude loss on masked patches.

    Motivation: standard MSE penalises low-frequency errors (blurry predictions).
    The FFT term penalises high-frequency errors (missing texture), which is
    critical for detecting fine-grained pathologies (nodules, fibrosis).
    """
    # Only compute on masked patches
    pred_m   = pred[:, mask]    # (B, n_masked, patch_dim)
    target_m = target[:, mask]  # (B, n_masked, patch_dim)
...
def pretrain(args):
    ...
    scaler = GradScaler(enabled=use_amp)   # BUG-1 pattern: no device positional arg
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for imgs in tqdm(loader, desc=f"SSL Ep {epoch:03d}", leave=False):
            imgs = imgs.to(device)
            mask = anatomy_guided_mask(num_patches, IMG_SIZE, PATCH_SIZE, MASK_RATIO)
            mask = mask.to(device)
            opt.zero_grad()
        ...
        if epoch % 20 == 0 or epoch == args.epochs:
            ...
            torch.save(
                {"epoch": epoch, "backbone": mae.encoder.state_dict()},
                ckpt_path,
            )
            print(f"  Saved SSL checkpoint → {ckpt_path}")

    # Save final backbone only (for train_v3.py --ssl_checkpoint)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wb_entity",  default="",
                        help="Weights & Biases username (e.g. Zeesejo)")
